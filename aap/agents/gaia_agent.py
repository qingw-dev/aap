...
class GaiaAgent(Agent):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            conf=config, name="GaiaAgent", resp_parse_func=resp_parse_func, **kwargs
        )
        ...
        self._color_log(
            f"Using {config.llm_model_name} from {config.llm_base_url}", Color.red
        )
    ...
    def policy(
        ...
        self.desc_transform()
        images: list[str] | None = observation.images if self.conf.use_vision else None
        if self.conf.use_vision and not images and observation.image:
            images: Any | None = [observation.image]

        agent_result, llm_response = self._call_llm_for_agent_result(
            content=observation.content
        )

        answer_match: re.Match | None = re.search(
            r"<answer>(.*?)</answer>", llm_response.content, re.DOTALL
        )
        if not agent_result.is_call_tool and not answer_match:
            self._color_log(
                message="⚠️ No Answer & No ToolCall, Try Again!", color=Color.cyan
            )
            agent_result, llm_response = self._call_llm_for_agent_result(
        ...
        for action in actions:
            if action.action_name is not None:
                ...
                _log_acts.append(
                    f"🔨 func={action.action_name} 🧾 arguments={call_args}"
                )
            else:
                _log_acts.append(f"💯 result={action.policy_info}")

        _log_policy: LiteralString = "\n".join(_log_acts)
        self._color_log(message=f"💡 Policy: {_log_policy}", color=Color.cyan)

        return actions

    def _call_llm_for_agent_result(
        self, content: str
        ...
        try:
            ...
            llm_response: ModelResponse | Generator[ModelResponse] = call_llm_model(
                self.llm,
                messages=messages,
            ...
            self._color_log(
                f"🤖 Execute response: {llm_response.message}", Color.orange
            )
    ...
    def _setup_logger(
        self, logger_name: str, output_folder_path: str, file_name: str = "app.log"
    ) -> logging.Logger:
        return setup_logger(
